...
func (lp *Loadpoint) setLimit(current float64) error {
	...
	if lp.circuit != nil {
		// custom: priorities, switch devices and the shed guard, see core/loadpoint_lm.go
		circuit, requested := lp.lmCircuit(), current
		...
		circuit.done(requested, current) // custom
	}
